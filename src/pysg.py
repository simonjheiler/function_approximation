...
class gridPoint:
    ...
    def pointPosition(self, index, domain=None):
        coord = []
        if domain is None:
            for i in range(math.ceil(len(index) / 2)):
  ...
  """
    ...
    def evalFunct(self, x):
        ...
        for i in range(self.dim):
            ...
            for j in range(1, self.level + 1):
                ...
                self.evalPerDim[i][j - 1].append(
                    evalBasis1D(x[i], self.evalPerDim[i][j - 1][0], self.domain[i])
                )
    ...
    def generatePointsRec(self, dim, level, cur_level=None):
        ...
        if dim == 1 and cur_level == level:
            ...
            return cross(
                basis_cur, self.generatePointsRec(dim - 1, level - cur_level + 1)
            )
        else:
            # crossproduct of this dim indices and other (dim-1) ones
    ...
    def nodal2Hier1D(self, node, i, j, dim):
        ...
        if left[2 * dim] == 0:
            ...
            self.gP[tuple(index)].hv -= 0.5 * (
                self.gP[tuple(left)].hv + self.gP[tuple(right)].hv
            )
