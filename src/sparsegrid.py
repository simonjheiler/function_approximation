...
def _clenshaw_interpolation(
    n_dimensions, interpol, residuals, output_size, nodes, indx, grid_out, grid_in
):
    """
            ...
            for dim in range(n_dimensions):
                ...
                if nodes[indx[level, dim]] != 1:
                    for node in range(nodes[indx[level, dim]]):
                        xtmp = 0.5 * (
                            1.0
                            + (-np.cos((np.pi * node) / (nodes[indx[level, dim]] - 1)))
                        )
                        # Transform xtmp based on interval
                        delta = np.abs(
                            np.min(interpolation_interval[:, dim])
                        ...
                        if np.abs(grid_in[level, dim] - xtmp) > 1.0e-03:
                            val = (grid_out[i, dim] - xtmp) / (
                                grid_in[level, dim] - xtmp
                            )
                            polyw[level, dim] *= val
    ...
    for i in range(2, nsamples + 2):
        ...
        for j in range(1, nnodes[i - 1] + 1):
            if interpolation_type.lower() == "ch":
                xit.append(
                    (1 + (-(np.cos((np.pi * (j - 1)) / (nnodes[i - 1] - 1))))) / 2.0
                )
            else:
...
def get_multi_index_sequence(nsamples, dimensions):
    ...
    levels_it = it.combinations(range(nsamples + dimensions - 1), dimensions - 1)
    nlevels = 0
    for _ in levels_it:
        nlevels += 1
...
def sparse_interp(
    ...
    if interpolation_type.lower() == "cc".lower():
        _clenshaw_interpolation(
            n_dimensions, interpol, residuals, output_size, nodes, indx, grdout, grdin
        )
    elif interpolation_type.lower() == "ch":
        _chebyshev_interpolation(
            n_dimensions,
            interpol,
            residuals,
            output_size,
