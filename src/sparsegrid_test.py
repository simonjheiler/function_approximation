...
2D Clenshaw-Curtis piece-wise linear basis sparse grid interpolation of 2D test
    function in fun_nd
...
def make_data():
    ...
    n = 6  # Maximum degree of interpolation to consider
...
def fun_nd(x):
    ...
    func2d = (
        (
            0.5 / np.pi * x[:, 0]
            - 0.51 / (0.4 * np.pi ** 2) * x[:, 0] ** 2
            + x[:, 1]
            - (0.6)
        )
        ** 2
        + (1 - 1 / (0.8 * np.pi)) * np.cos(x[:, 0])
        + 0.10
    )
    return func2d
...
def plot_result(gridout, output, shape, X, Y, func):
    ...
    ax.plot_surface(
        X, Y, (out_vals - ref_vals), rstride=1, cstride=1, cmap=plt.cm.magma
    )
...
if __name__ == "__main__":
    ...
    if chebyshev is True:
        ...
        # Run Chebyshev polynomial sparse-grid interpolation of 2D test function
        # in fun_nd
    ...
    if clenshaw is True:
        ...
        # Run Clenshaw-Curtis Piece-wise linear sparse-grid Interpolation of 2D
        # test function in fun_nd
